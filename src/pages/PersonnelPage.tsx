...
import { useStorage, type TaskPersonnel } from '../context/StorageContext';
...
export const PersonnelPage: React.FC = () => {
  ...
  const { getPersonnel, savePersonnel, deletePersonnel } = useStorage();
  ...
  return (
    <div className="container" style={{ paddingBottom: '2rem' }}>
      <header style={{ marginBottom: '1.5rem', marginTop: '1rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
        ...
        <div>
          <h1 style={{ fontSize: '1.5rem', fontWeight: '700' }}>Technicians & Laborers</h1>
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>Manage workforce and personnel</p>
        </div>
      </header>
            ...
            {personnel.length === 0 ? (
              ...
              personnel.map(p => (
                <div 
                  key={p.id} 
                  className="card" 
                  onClick={() => navigate(`/personnel-tasks/${p.id}`)}
